<div x-data="{
            ...
            <div class="bg-white rounded-xl shadow-2xl p-6 w-full max-w-3xl mx-4 relative max-h-[90vh] overflow-y-auto">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-gray-800 animate-fade-in">
                    ...
                    <div>
                        ...
                        <p class="border rounded px-3 py-2 bg-[#FFF5F3]">{{ $selectedPeserta->user->nim_magang ?? '-' }}
                        </p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="border rounded px-3 py-2 bg-[#FFF5F3]">{{ $selectedPeserta->durasi_magang ?? '-' }}
                        </p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="border rounded px-3 py-2 bg-[#FFF5F3] capitalize">
                            {{ ucwords(str_replace('_', ' ', $selectedPeserta->status)) }}</p>
                    </div>
                    <div>
    ...
    <div x-data="{
        ...
        <div class="flex flex-wrap justify-between items-center gap-4 py-2">
            {{-- <button wire:click="generateAllNim" wire:loading.attr="disabled" wire:target="generateAllNim"
                    title="Generate NIM untuk peserta yang belum memiliki NIM (status: accepted, on_going, done)"
                    class="inline-flex items-center px-3 py-2 bg-gradient-to-r from-blue-500 to-blue-600 text-white text-sm font-medium rounded-lg shadow-md hover:from-blue-600 hover:to-blue-700 transform hover:scale-105 transition-all duration-200 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed">
            Generate NIM</button> --}}
            {{-- Filter status (kiri) --}}
            <div class="flex flex-wrap gap-3 items-center">
                ...
                @foreach ($filters as $key => $f)
                    <button wire:click="setFilter('{{ $key }}')" wire:loading.attr="disabled"
                        wire:target="setFilter" type="button"
        ...
        <div class="bg-white rounded-lg overflow-x-auto border my-5 relative">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-3 text-left text-sm font-semibold text-gray-700 uppercase tracking-wide">NIM
                        </th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($peserta as $p)
                        <tr class="hover:bg-gray-100 transition-colors duration-200">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $p->user->name ?? '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                ...
                                @endphp
                                <span
                                    class="inline-flex px-2 py-1 rounded-full text-xs font-semibold {{ $statusColors[$p->status] ?? 'bg-gray-200 text-gray-700' }}">
                                    {{ ucwords(str_replace('_', ' ', $p->status)) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $p->unit_penempatan }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $p->durasi_magang }}
                                bulan</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex justify-center space-x-2">
                                    <button wire:click="confirmApprove({{ $p->id }})"
                                        wire:loading.attr="disabled" wire:target="confirmExecute" type="button"
                                    ...
                                    <button wire:click="confirmReject({{ $p->id }})"
                                        wire:loading.attr="disabled" wire:target="confirmExecute" type="button"
                                    ...
                                    @if (
                                        ...
                                        <button wire:click="confirmCancelStatus({{ $p->id }})"
            ...
            <div wire:loading.flex wire:target.live="search,setFilter,triggerSearch"
