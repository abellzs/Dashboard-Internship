...
>
        ...
        <div class="bg-white rounded-xl shadow-2xl p-6 w-full max-w-3xl mx-4 relative max-h-[90vh] overflow-y-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-gray-800 animate-fade-in">
                ...
                <div>
                    ...
                    <p class="border rounded px-3 py-2 bg-[#FFF5F3]">{{ $selectedPeserta->user->profile->instansi ?? '-' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-semibold mb-1">Jurusan</label>
                    <p class="border rounded px-3 py-2 bg-[#FFF5F3]">{{ $selectedPeserta->user->profile->jurusan ?? '-' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-semibold mb-1">Jenis Kelamin</label>
                    <p class="border rounded px-3 py-2 bg-[#FFF5F3]">{{ $selectedPeserta->user->profile->jenis_kelamin ?? '-' }}</p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <p class="border rounded px-3 py-2 bg-[#FFF5F3]">
                        {{ $selectedPeserta?->tanggal_selesai_usulan ? \Carbon\Carbon::parse($selectedPeserta->tanggal_selesai_usulan)->format('d M Y') : '-' }}</p>
                </div>
                <div>
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden animate-fade-in duration-300 py-3 px-6">
        ...
        <div class="bg-white rounded-lg overflow-x-auto border my-5 relative">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th 
                            wire:click="sort('tanggal_mulai_usulan')" 
                        ...
                        <th 
                            wire:click="sort('tanggal_selesai_usulan')" 
                            class="cursor-pointer px-6 py-3 text-left text-sm font-semibold text-gray-700 uppercase tracking-wide select-none min-w-[140px]">
                            <div class="flex items-center space-x-1">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($magang as $row)
                        <tr class="hover:bg-gray-100 transition-colors duration-200">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $row->user->nim_magang ?? '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $row->user->name ?? '-' }}</td>

                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $row->unit_penempatan }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                ...
                                @endphp
                                <span class="inline-flex px-2 py-1 rounded-full text-xs font-semibold {{ $statusColors[$row->status] ?? 'bg-gray-200 text-gray-700' }}">
                            ...
                            <td class="px-6 py-4 flex justify-center space-x-2">
                                <button wire:click="viewDocument({{ $row->id }})" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded-full transition transform hover:scale-110">
            ...
            {{-- Loading overlay --}}
            <div wire:loading.flex wire:target="search,setFilter" class="absolute inset-0 bg-white bg-opacity-70 flex items-center justify-center rounded-lg" style="z-index:10;">
